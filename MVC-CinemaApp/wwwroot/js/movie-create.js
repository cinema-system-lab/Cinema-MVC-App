$(document).ready(function() {
    ...
    if ($posterUrlInput.length && $posterPreview.length) {
        $posterUrlInput.on('input', function() {
            const url = $(this).val();
            if (url && url.startsWith('http')) {
                $posterPreview.html(`<img src="${url}" alt="Movie Poster" onerror="this.parentElement.innerHTML='<i class=\\'bi bi-exclamation-triangle text-danger fs-1\\'></i>'">`);
            } else {
                $posterPreview.html('<i class="bi bi-image text-muted fs-1"></i>');
            }
        });
    }
    ...
    if ($ratingInput.length) {
        ...
        $ratingInput.on('blur change', function() {
            formatRating($(this));
        });
